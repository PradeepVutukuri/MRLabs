...
public class MovingAverageJob extends Configured implements Tool {
	public static class MovingAverageReducer extends Reducer<Stock, DoubleWritable, Text, DoubleWritable> {
		...
		InterruptedException {
			for(DoubleWritable value : values) {
				StockWindow window = new StockWindow(key.getSymbol());
				windows.add(window);
				for(StockWindow w : windows) {
					w.addPrice(value.get(), key.getDate());
				}
				if(windows.size() >= WINDOW_SIZE) {
					StockWindow first = windows.removeFirst();
					outputKey.set(first.toString());
					outputValue.set(first.getAverage());
	...
	public int run(String[] args) throws Exception {
		Job job = new Job(getConf(), "MovingAverageJob");
